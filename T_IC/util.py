...
def set_universe(block_name, dates, dict_name_code):
    '''
...
def get_factor_loadings(univ,fac,v,date,file_prefix = 'F:/factors/'):
    '''
    ...
    if v is not 'a':
        ...
        for  k in fac.keys():
            factor_loadings = pd.read_csv(file_prefix+'fac_value.csv',usecols=['date','secID',k])
            factor_loadings.dropna(inplace=True)
            #alter
            b = [0]
            factor_loadings = factor_loadings[-factor_loadings[k].isin(b)]
            #factor_loadings = factor_loadings.drop_duplicates(subset=['date','secID'], keep='first')
...
def get_factor(univ, fac, date, file_prefix):
    '''
    ...
    if fac in ['PB','PE','PS']:
        if fac == 'PE':
            ...
            pass
        d = w.wss(codes=univ, fields=["windcode", fac], tradeDate=date, ruleType=rt)
        temp = {
        ...
        if fac == 'PE' or fac == 'PB' or fac == 'PS':
            factor[fac] = 1.0 / factor[fac]
        factor.dropna(inplace=True)
        factor.set_index('secID', inplace=True)
        return factor
    else:# readcsv
        factor = pd.read_csv(file_prefix+'fac_value.csv',usecols=['secID','date',fac])
        factor.dropna(inplace=True)
        factor.set_index('date', inplace=True)
        factor = factor.loc[date]
        factor['secID'] = replace_suffix(fac['secID'].tolist())
        factor.set_index('secID', inplace=True)
        return factor